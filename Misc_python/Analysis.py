...
def Hydrostatic():
    ...
    plt.figure(figsize=(8.5, 5))
    plt.scatter(z, p[::-1], color = 'r', label = 'SPH pressure')
    plt.plot(z_th+dist, p_th, color = 'b', label = 'Theoretical hydrostatic pressure')
    ...
    if(save):
        current_directory = os.path.dirname(os.path.abspath(sys.argv[0]))
        plt.savefig(f'{current_directory}/Pictures/2D_hydrostatic')
    plt.show()
...
def dam_break(): 
    ...
    plt.plot(time_ad, pos_ad , color = "red", zorder = 1)
    plt.xlim(0,4)
    plt.ylabel(r"x/L [-]")
    plt.xlabel(r"t$\sqrt{2\,g/L}$ [-]")
    plt.grid(True, alpha = 0.5)
...
def MRUA():
    ...
    plt.scatter(time[0], pos_z[0] , color = "red", marker = "o", label = "SPH simulation Euler" , zorder = 2)
